...
- **The public reader is `Reader2C` (updated 2026-08-30):** `/book/[id]/page/[pageId]` renders `src/components/reader-v2/Reader2C.tsx` (chosen in `(reader)/page.tsx`); `TranslationEditor.tsx` (via `PageEditorClient`) survives only for tenant/BPH embeds (`ctx.isEmbedded`). A reader-facing fix usually needs Reader2C, and an embed-facing one still needs TranslationEditor — despite its `src/components/pipeline/` path it is NOT internal tooling (that misread nearly left #3157 undone). Its footer owns LikeButton + nav hint + BookSearchBar; comments UI was removed after the #3093 probation ended with `annotations` at zero docs ever (`/api/annotations` routes kept).
