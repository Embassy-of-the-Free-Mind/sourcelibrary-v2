...
- **A ghost verdict comes from `batches.get` on the job, never from absence in a listing (2026-09-16, #4889):** the reconcile step in `batch-collector.mjs` used to walk `batches.list` newest-first, stop after 50 consecutive inactive jobs, and write every DB-active job outside that window as `failed: Ghost ... 404` once it was >30 min old — none had returned 404. Under load (the #4541 drain) 85 RUNNING file-batch OCR jobs / 8,770 pages were failed that way, resubmitted by the drain, then completed and billed on Gemini uncollected. The logic now lives in `scripts/workers/lib/batch-reconcile.mjs` (pinned by `tests/unit/batch-reconcile-ghost.test.ts`): a candidate is PROBED on every key; only a real 404 everywhere is a ghost; a network/quota/5xx on any key is `unmeasurable` and the row is left in flight; the listing walk is a logged page budget (20 pages/key); orphan cancellation skips any `job_name` the DB knows in ANY status (a failed row is not an orphan — cancelling it destroys paid work); the per-key evidence is written on the row as `ghost_verdict` / `last_ghost_probe` (key fingerprints, HTTP status). The same distinction applies to the per-job collection path: `getJobData` now returns `not_found` only on 404-everywhere. **Tell of the old bug:** `error: /Ghost/` rows whose `job_name` returns `BATCH_STATE_RUNNING` from `GET /v1beta/batches/<name>` with the creating key. Recovery for rows ghosted before the fix: ask Gemini directly and flip RUNNING/PENDING/SUCCEEDED back to `processing` (the drain's un-ghost step); the collector's own recovery pass only resurrects SUCCEEDED.
