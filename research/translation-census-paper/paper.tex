...
\small
\centering
\caption{Estimated translation gap by tradition. Percentages are rough estimates based on catalogue analysis and existing translation series; methodology notes below.}
\label{tab:gap}
\begin{tabular}{@{}lrl@{}}
\toprule
Tradition & Surviving corpus & Translated to English \\
\midrule
Latin (classical, to 600) & 150M words & 30--50\%$^a$ \\
Latin (medieval/early mod.) & billions of words & $<$1\%$^b$ \\
Sanskrit & 30M MSS$^c$ & $\ll$1\% \\
Arabic & 3--7M MSS & $\ll$1\% \\
Classical Chinese & 5B+ characters & $<$5\%$^d$ \\
Ancient Greek (classical) & 150M words & 30--50\%$^a$ \\
\bottomrule
\end{tabular}
\end{table}

{\footnotesize \noindent $^a$Canonical authors well covered by Loeb, Bud\'e, Tusculum, Oxford Medieval Texts, Penguin Classics, DOML, I Tatti Renaissance Library, and individual scholarly editions. The 30--50\% estimate applies to \emph{surviving works}, not words or pages. $^b$The vast post-600~CE Latin corpus (millions of sermons, legal texts, scientific treatises, administrative documents) has received very little systematic English translation. $^c$Pingree's estimate is of physical manuscripts, not distinct works; many are copies. The number of distinct Sanskrit works is unknown but far smaller. $^d$Major philosophical and literary works are translated; technical, administrative, and local texts mostly are not.}

\smallskip
For the canonical classical authors (to ${\sim}$600~CE), existing translation series provide reasonable coverage---perhaps 30--50\% of surviving works in English. But this is the tip of the iceberg. The vast post-classical corpus---medieval Latin, early modern vernacular scholarship, manuscript traditions in Sanskrit, Arabic, and Chinese---remains overwhelmingly untranslated. The ``canon'' that shapes modern understanding of pre-modern thought is drawn from an extraordinarily narrow sample.
...
Of the 12,877 books with content, ${\sim}$9,100 are non-English texts that have been translated to English. We believe the majority are first English translations---most are obscure early modern texts unlikely to have attracted human translators---but we have not systematically checked against existing translation databases (Index Translationum, WorldCat). Some well-known works in our collection certainly have prior translations. The claim is strongest for the ``long tail'' of non-canonical texts. The peak of the collection falls in the 17th century (1,510 books from 1600--1649), reflecting the explosion of print culture in early modern Europe.
...
AI has substantially reduced this bottleneck. Multimodal LLMs (Gemini, Claude, GPT-4) can translate printed historical text at ${\sim}$\$0.002 per page with quality that, for well-represented languages like Latin and German, is often serviceable for discovery and first-pass comprehension~\cite{humphries2024,volk2024}. This is not the same as scholarly translation: AI output lacks textual criticism, source identification, historical commentary, and the decades of training that inform expert judgment. But for the millions of texts that will \emph{never} receive scholarly attention, AI translation provides access where none existed. The marginal cost reduction---from hundreds of dollars per page for expert translation to fractions of a cent---changes the economics of what is feasible to translate.
...
\textbf{Concrete next steps for this census:}

\begin{enumerate}
\item \textbf{Validate ``first translation'' claims.} Cross-reference Source Library's 9,100 non-English translations against WorldCat, Index Translationum, and the Digital Latin Library to determine how many are genuinely first English translations vs.\ duplicating existing work.
\item \textbf{Stratify translation quality by language.} Run the self-consistency evaluation~\cite{lomas2026ocr} across a sample of 100 books spanning all 15 major languages to identify which languages and periods produce reliable AI translations and which do not. Sumerian, Syriac, and Ge'ez are particular unknowns.
\item \textbf{Ground the percentage estimates.} For Latin, systematically compare Source Library's catalogue against the Loeb, Bud\'e, Tusculum, DOML, and I Tatti series to determine what fraction of our collection has existing expert translations. Extend to Greek (Perseus catalogue), Arabic (Brockelmann/Sezgin), and Chinese (ctext.org coverage).
\item \textbf{Engage domain specialists.} Commission blind expert reviews of 10 AI translations per major language (Latin, German, Greek, Sanskrit, Arabic, Chinese) to establish baseline quality and identify systematic errors.
\item \textbf{Publish the union catalogue.} Release the 1.1 million candidate records as an open dataset with language, date, source library, and digitization status, enabling the scholarly community to prioritize translation targets.
\end{enumerate}
