...
Critically, we have not ablated the Hindi effect from generic chain-of-thought reasoning. The CoT prompt mentions Hindi; it is unclear whether the improvement comes from Hindi-specific knowledge or simply from the instruction to ``think first, then translate.'' A control condition (``analyze the grammar step by step'' with no language mention) is needed to isolate the cognate priming effect.

The LLM judge (Gemini evaluating Gemini translations) introduces same-family bias and cannot verify Sanskrit fidelity at the word level---a Sanskritist's evaluation is essential. The judge scales differed between Sanskrit (1--5) and Chinese (/10) experiments, an inconsistency that should be unified. A full study needs: (a)~larger corpus across domains, (b)~multiple models, (c)~human expert evaluation, (d)~COMET comparison against existing translations, and (e)~the ablation experiment described above.
