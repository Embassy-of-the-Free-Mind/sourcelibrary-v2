...
## 5. Instrument validation (in progress)

Two validation studies are designed and pending a clean execution window (they were interrupted by transient server-side rate-limiting when run concurrently with the main samples):

- **Verdict reliability.** A random subsample (n = 50) is re-adjudicated by independent agent runs; we report exact-verdict agreement, collapsed three-class agreement, and Cohen's κ. This bounds the stability of the language-model agent as a measurement instrument.
- **Ground-truth agreement.** The adjudicator is run against a 33-case human-vetted set (each label independently checked against five external catalogues) and agreement is reported. This bounds accuracy against known answers.

These results will be appended; the corpus estimates above do not depend on them, but the paper's claim that an LLM agent is a *usable instrument* does, and they are the highest-value remaining measurement.
