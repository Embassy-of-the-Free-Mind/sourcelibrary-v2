...
## Twin-route mechanics (learned 2026-08-20)

- **Route segment config must be a literal in the twin file.** `export { revalidate }
  from '…/page'` and a re-exported `preferredRegion` fail the Turbopack build
  (Next parses them statically). Re-export the component; restate the config.
- **A new `books` field is declared in TWO places:** `scripts/lib/book-docs.mjs`
  (constructors) and `scripts/lib/books-known-fields.json` (the field-write lint
  in CI). `localized` and `pages_translated_es` are in both.
- **The collection grid is the Supabase mirror.** Tagging books into a collection in
  Mongo shows "0 books" until `updated_at` is bumped, `sync-books-catalog.mjs`
  runs, and the ISR entry is revalidated + edge-purged.
- **Audit a translation RELATIVE to its source page.** Absolute checks (repetition,
  English share) flagged only false positives; the same checks against the English
  page found the real defects — and several were runaway *English* pages (#4098).
