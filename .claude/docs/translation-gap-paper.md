...
How much of the early-modern Latin record has ever been translated into English? The question underlies a common claim — that the vast majority never has — but the claim is usually asserted, not measured. We present a reproducible, work-level pipeline that answers it against the Universal Short Title Catalogue (USTC): we collapse 499,604 Latin editions printed 1400–1700 into **366,205 distinct work-clusters**, cross-match them at the *work* level (author-anchor + rarity-weighted title containment, not the author-level flag that over- and under-counts) against an assembled catalogue of known English translations drawn from UNESCO's *Index Translationum*, the Library of Congress, OpenLibrary, HathiTrust and the major scholarly series, and count a work as translated **only on evidence independent of our own project**. The apparent result is that 2.7% have a prior English translation. We then convert that apparent rate into a *validated* one. Drawing a blind, two-sided stratified sample (n=250: 120 pipeline-"translated", 130 pipeline-"gap", window 1480–1620), we adjudicate each work with **two independent grounded-search instruments** (a Claude tool-using agent and a Gemini grounded-search call) sharing one verdict taxonomy, resolve their disagreements with a third strict-completeness pass, and **post-stratify by composition era** — because USTC's year is the *print* year, so the print-Latin corpus includes early-modern reprints of ancient and medieval authors. The two instruments agree on the binary translation-exists question 94% of the time (Cohen's κ=0.88). The decisive finding is that **the denominator dominates**: for the *all-print* corpus the debiased gap is **82.4% [75.8–88.2]** — *lower* than the raw 95.9%, because the pipeline misses English translations of the ancient/medieval classics reprinted in the window (only 42% of "gap" ancient/medieval works are truly untranslated) — whereas for **genuinely Renaissance-composed Latin works the debiased gap is 95.6% [91.6–98.7]**, essentially total, corroborating and slightly exceeding Debora Shuger's standing "roughly 90%" estimate, now *measured* with error bars. We also find the matcher's precision is era-dependent (91% on classics, 61% on Renaissance works), so the raw translated count is inflated precisely where the gap matters. The contribution is the integrated, auditable method and the released work-graph dataset; the figure is a *result* of that method, reported as an interval with per-item search provenance, never as a bare percentage.
...
**Sampling.** From the 1480–1620 window we drew, with a fixed seed (reproducible), **120 pipeline-"translated"** clusters (to measure matcher precision) and **130 pipeline-"gap"** clusters (to measure recall / the true untranslated rate). Labels and matched sources were stripped and the order shuffled before adjudication; each work reached the adjudicators as only {author, title, print year, edition count}.

**Two independent grounded instruments**, sharing one verdict taxonomy, each blind to the pipeline label and to each other:
- **Claude tool-using agents** (primary), one per work, run as a fan-out workflow (`scripts/analysis/gap-validation-claude-workflow.mjs`); each does real `WebSearch`/`WebFetch` over Google Books, archive.org, HathiTrust, WorldCat, and scholarship, and returns a structured verdict plus its queries and the source URLs it consulted. 250/250 returned grounded verdicts (one transient rate-limit, re-run).
- **Gemini grounded-search calls** (`gemini-3-flash-preview`, `googleSearch` tool; `scripts/analysis/gap-validation-gemini-adjudicate.mjs`), one per work, same taxonomy. 250/250 grounded (12 initial memory-only answers were re-run with grounding restored — the grounding-suppression failure mode documented in the FT audit).
...
On the binary "does a prior English translation exist", Claude and Gemini agreed on **94%** of works, Cohen's **κ=0.88** ("almost perfect"). Of 250 works, 10 were binary disagreements (resolved by the third pass — most were partial-vs-complete grading: Melanchthon's 1522 *Annotationes* has only its 1 Corinthians portion in English, Reisch's *Margarita philosophica* only 4 of 12 books, Della Porta's *De distillatione* only Book 1 — all correctly **untranslated** under the completeness rule) and 38 were `container`/ill-posed (multi-work *Opera*, excluded). The strict count is reproducible across independent model families; the residual disagreement is taxonomy grading, not search failure — the same decomposition the FT audit found.

### 5.2 Matcher precision is real but era-dependent

Of pipeline-"translated" works (well-posed), **78.3% [69–85]** have a confirmed real prior English translation. But precision splits sharply by era: **90.7%** for ancient/medieval classics (49/54) versus **60.5%** for genuinely Renaissance-composed works (23/38). The Renaissance false positives are low-IDF title collisions (a school commentary matched to the classic it glosses; a shared rare word). Consequence: the raw translated count is inflated *precisely* on the Renaissance works where the gap is the point.
...
|---|---|
| All print-clusters | 85.0% [78–90], n=120 |
| **Renaissance-composed only** | **96.8% [91–99], n=95** |
| Ancient/medieval reprints only | 41.7% [24–61], n=24 |

The ancient/medieval cell is the confound, measured: **58% of "gap" ancient/medieval works are in fact translated** — the matcher misses them (they are translated under different titles, in Loeb/series the matcher does not fully enumerate), but they are translated *because they are classics*. They make up ~22% of the gap stratum and drag the all-print figure down.

### 5.4 Debiased population estimate

Combining both strata, weighted by population and post-stratified by era (bootstrap 95%):

> **Debiased gap — all-print denominator (1480–1620): 82.4% [75.8–88.2]** (translated 17.6% [11.8–24.1]).
> **Debiased gap — Renaissance-composed denominator: 95.6% [91.6–98.7]** (translated 4.4% [1.3–8.4]).
...
This **corrects and sharpens the n=30 pilot**, which reported a Renaissance gap of ~86% (95% CI 60–96, n=14) and concluded the raw figure "overstates the Renaissance gap." At n=95 the Renaissance gap is ~96%; the pilot's lower point estimate was small-sample noise (2 misses in 14). What the raw figure overstates is the *all-print* gap, not the Renaissance one. The validated headline is therefore: **~96% of Renaissance-composed Latin works are untranslated** — measured, with error bars, corroborating Shuger's "90%."

### 5.5 Provenance is auditable, and reaches the right sources

Every verdict stores the domains it consulted (1,199 source hits across the Claude pass). The mix is authoritative-heavy: en.wikipedia (147), archive.org (102), Loeb Classics (34), Cambridge (23), Google Books (48), HathiTrust (19), Michigan/Penn online-books, Deutsche Biographie, Wikidata, New Advent. Notably the Claude agent **reaches WorldCat** (43 hits) where the FT audit's Gemini pass was 403-blocked — a concrete instance of *where* a model looks mattering as much as *which* model looks, and of the value of an independent second instrument with a different search surface.
...
- the **n=250 validation set**: blind frame + answer key, both adjudicators' full verdicts with queries and source URLs, the tie-break, the scorer/estimator, and the human-gold export — all under `scripts/analysis/eval-data/`, reproducible from the seed.
