...
| **Preview** | Phase 1.5 preview OCR (first 25 pages, **Batch API**, pooled cross-book, partitioned by script), post-archive only | **Yes** (via orchestrator) | Gets metadata/classification before full OCR. The import-time half of this lane was REMOVED 2026-08-30: `preview-ocr.ts`/`preview-translate.ts` fired straight onto SQS/Lambda from the import routes, gated by neither the pause nor the dial, and billed ~$392 in four days through a pause set to stop exactly that. Imports now spend nothing; first OCR comes from the orchestrator. |
