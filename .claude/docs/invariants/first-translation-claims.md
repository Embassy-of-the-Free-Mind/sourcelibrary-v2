...
- **A "found" from `gemini_verifier` is NOT a search result (#4525, measured 2026-09-01).** Paired on the **5,534 books both it and `gemini_grounded_search` judged** — same books, so no population confound — it says FOUND where grounded says NONE on 1,424 (25.7%), against only 186 (3.4%) the other way: a **7.7× asymmetry**, agreement 70.9%, found-rate 48.2% vs 25.8%. Its rows *do* carry queries (95.2%) and sources; they are **recorded, not necessarily executed**, and a written query string looks like retrieval while behaving like recall. It produced ~57K of the ledger's rows, so most of the corpus's prior evidence has this bias baked in.
