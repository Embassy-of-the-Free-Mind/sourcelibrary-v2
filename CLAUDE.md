...
- **Use `scripts/lib/revision-pairs.mjs` — never re-implement the filter.** It has TWO halves and each is invisible to the other: the printed `<page-num>` catches an *unlabelled* image swap, and the `source` label catches the repair sweep itself, whose moved text often prints the same number as the page it landed on. `hard-page-sample.mjs` carried only the page-number half; `revision-agreement-corpus.mjs` and `disagreement-typology.mjs` carried neither (fixed in #3558). `calibration-scorecard.mjs` reads the corpus summary and inherits whatever that script did. A per-**book** verdict sits on top, because a uniform slide preserves the printed sequence exactly, so unnumbered leaves in an affected book are invisible per-pair.
- **Full doc: `.claude/docs/double-ocr-dataset.md`** — what the dataset is, the five exclusions, what it can and cannot support (coverage is 0.67% of the OCR'd corpus, non-randomly selected), the convention/reading decomposition, and how to ask humans the right question.
- **A revision is not a second read, and the inventory is a separate question from the metric.** `scripts/eval/double-ocr-pages.mjs` answers *which pages carry two or more independent model passes over the same leaf* (also `--field=translation`; `--write-collection` materializes `double_ocr_pages` / `double_translation_pages`). It reports `verified-same-leaf` and `unverified-leaf` separately — most pages print no page number at all, so the total is NOT all verified and must never be quoted as if it were.
- **The TRANSLATION side is worse and nobody had looked (#3558).** The same sweep wrote 55,272 translation revisions — **41.1%** of that corpus against 29.5% on the ocr side. Sampled at 600 rows across 150 books: 95.2% match the *neighbouring* leaf's current text at Jaccard **1.00** (byte-identical), and **zero** are same-page pairs. Metadata gives no tell — same model, same `prompt_version`, and the revision's `original_date` equals the live text's `updated_at`. The clean lane exists: `source: 'ai'` is 81.5% genuine same-page re-translations.
...
**A reference set's SIZE is not its COVERAGE — and that, not the matcher, is the real limit.** The extract holds **1,230 pre-1800 imprints of 118,352 (1.04%)** while our corpus is overwhelmingly early-modern: we were asking a *modern-imprint catalogue* about *early-modern translations*. Every miss checked in the 2026-08-01 sample was **absent from the set, not mis-matched** — Sanford's 1569 Agrippa, Hall's 1654 Maier, Ellistone's 1651 Böhme, and Caplan's Loeb *Rhetorica ad Herennium* (zero rows). 118,352 records looks authoritative and is ~1% relevant to the question. **Before trusting a null, ask what fraction of the set falls in the period, language and population you are actually asking about.** Matcher/threshold work is finished; the next gain is sources (#3522, ESTC). Related: **depth is not reachability** — Chinese looks deep at 3,868 rows and is unanswerable anyway, because CJK is reachable only via MARC 880 (2.3% of rows).
