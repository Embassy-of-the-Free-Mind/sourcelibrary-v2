...
**A reference set's SIZE is not its COVERAGE — and a DATE COLUMN is not the date you think it is.** The extract holds **1,230 pre-1800 imprints of 118,352 (1.04%)**, and that figure was first read as "the set is ~1% relevant to an early-modern corpus." **It is not.** `year` records when the *translation* was printed, not when the original was written, and the English translation of a 1531 Latin work is usually a modern imprint — so **91% post-1950 is the correct shape** for this question, not a defect. Re-verified 2026-08-02: of the seven works named as misses, **six are genuinely absent, one (Agrippa's *De incertitudine*) is present** — LCCN 92246639, 1694, matched on its MARC 240 — and its books read `inconclusive`, i.e. surfaced-but-unscreened, never a miss. The real gaps are narrower and each has its own fix: **early-modern English printing** (Sanford 1569, Ellistone 1651, Hall 1654 — ESTC's range, #3522), **our own `041$h` hard filter** (`ingest-loc-bulk.mjs` drops any record with no `041`, which is why Caplan's Loeb *Rhetorica ad Herennium* has zero rows), and **source-language depth vs our holdings** (Latin 3,878 rows against 6,506 visible Latin books). **Before trusting a null, ask what fraction of the set falls in the period, language and population you are asking about — and which event the date field records.** Do not cite that sample as proof that only sources remain: one of five rows was an author-name search reported as a corpus absence. Related: **depth is not reachability** — Chinese looks deep at 3,868 rows and is unanswerable anyway, because CJK is reachable only via MARC 880 (2.3% of rows).
