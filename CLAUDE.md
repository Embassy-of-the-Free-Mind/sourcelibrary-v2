...
- **On PRs, trust `test`/DCO, not the Vercel result.** Wait for the real deployment to settle; don't bail or assume a real failure while `test`/DCO are green. **The mechanism is not a first-build-then-retry** — that was the earlier reading and it does not match the commit statuses. Observed on three consecutive PRs on 2026-08-07 (#3702, #3713, #3715): GitHub receives **two** Vercel statuses with the *same* `created_at` second — a `failure` pointing at a generic `vercel.link/<id>` that is **identical across unrelated PRs**, and a `pending` pointing at this PR's real deployment. The real one builds for 5–6 min and then passes. So the "fail" is not this PR's build failing first; it is a spurious status posted alongside it, and there is nothing to "retry". Check the deployment itself rather than the check name: `gh api repos/{owner}/{repo}/commits/$SHA/statuses` and look for a `pending`/`success` whose `target_url` is a real `vercel.com/...` deployment, or `npx vercel inspect <id>` for `status ● Building`.
...
- **Worktrees isolate checkouts. NOTHING isolates production Mongo — claim the issue before you build.** The rule above protects a file from git; this one protects a collection from another session. On 2026-08-07 two sessions independently implemented #3661 (canonical loci) within the same hour, and both rebuilt the derived `locus_anchors` collection with `deleteMany({})` + `insertMany`. One applied at ~23:10Z, the other at 23:19:43Z; **ordering alone decided which survived**. Reversed, the later sweep would have replaced 6,324 richer anchors with 4,279 poorer ones — and because the two schemas differed (`ref_page`/`ref_section` vs `page`/`section`), the just-shipped `/api/locus` would have queried absent fields and returned **zero for every locus, with no error and no failed write**. Green CI, live feature, silently answering nothing. Both halves are cheap to prevent: **(a)** post "taking this" on the issue *before* working — the third recurrence of that lesson, and the trigger each time is that the task felt like continuation rather than a fresh pickup, when in fact a well-scoped open issue is exactly what another session also grabs; **(b)** before any `--apply` sweep that clears a whole derived collection, read what is already there and refuse on an unexpected `extractor_version` rather than replacing it silently. #3714. Per-book clears (`deleteMany({book_id})`) are far safer than global ones — the audit question is "which sweeps clear globally".
