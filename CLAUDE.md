...
- **Pause `entities` bulk sweeps before any prod build — and a merge IS a prod build.** Check with `node --env-file=.env.production.local scripts/audit/entities-sweep-active.mjs` (exit 2 = UNKNOWN, not clear). **Tell:** `Error occurred prerendering page "/explore"` in the build log. Mechanism → `deploy-and-caching.md`.
...
- **A worktree isolates the FILES. It does not isolate production.** Mongo, Supabase and R2 are shared by every session, so a new collection is shared mutable state the moment you name it, and two sessions can each believe they own it. **Claim the issue before starting** (a "taking this" comment). **A row count is not an integrity check** — assert the SHAPE. And a write-time guard cannot help, because the clobbering writer is another session's script and never runs your guards: put the check on the READ side, or in an audit both sides run. **Tell:** a feature that reads as merely *empty* rather than broken. Incident (#3661, two schemas, 6,324 rows replaced by 4,279) → `main-checkout-and-worktrees.md`.
...
- **A page image key MUST contain its own `book_id`.** A book-independent key is shared between books *by construction* and **nothing fails loudly** — R2 serves a real, complete, 200-OK JPEG, so every "fail closed if the fetch fails" check passes. #3362 wrote 300 books to `archived/undefined/<page_number>.jpg` and OCR then transcribed *other books' pages* into them (130,040 pages). Guards are at the write boundary — `validateR2Key` / `assertBookScopedKey` (`scripts/lib/r2-key.mjs`, `src/lib/r2-key.ts`) called from `storagePut()` and the bulk page-image writers (#3365). Don't add a new R2 key site without one, and **don't guard a single helper and call it done** — that is how the identical bug shipped twice in one week. Standing detector: `scripts/audit/r2-key-book-scope.mjs`. Repairing the images and the text is **not the whole repair** → `derived-metadata-lane.md`.
...
- Production database: `bookstore`, NOT `sourcelibrary_research`. Measured 2026-09-17: **114,695** total docs, **57,670** `visible: true` — but **15,751 of those are artwork records with `pages_count: 0`, so the honest "books you can read" number is 41,919** (`visible: true && pages_count > 0`). Also **89,700** with `pages_count > 0` (actually processed), **64,444** with `pages_ocr > 0` (21.6M pages ingested, 6.77M transcribed, 5.07M translated). **Re-measure before quoting** — the previous vintage of this line was three weeks old and already 24% low on the reader-facing count (it said 31,731 against a true 41,919), and the one before that was off by 5×. The `tier` field is legacy (only `src/app/page.tsx` homepage ranking, via `highlighted_books` entries); the canonical "live" filter across all public APIs is `visible: true && pages_count > 0` (see `/api/books/library`).
...
- Declaring a bad-read repair finished, or writing anything that DERIVES a field from page text (`ai_metadata`, `summary`, `chapters`, `quality_score`) → `derived-metadata-lane.md` (**a poisoned read corrupts THREE lanes — image, text, and everything derived from text; the lane-1 detector returns PASS while lane 3 serves another book's title to readers**)
...
- Pipeline phases, `pipeline_auto.status`, `setPipelineStatus`, "assert the stage really ran" guards, **or keeping a named book out of every lane** (`held`, `scripts/lib/pipeline-hold.mjs`) → `pipeline-status-truth.md`
...
- **Every incident handoff ends with a CLAUDE.md check, in BOTH directions, and a check beats a sentence.** *Before either:* could this be a sweeping test, a detector, or a constructor that throws? A doc is the weakest layer — it only works if the next person reads it at the right moment. Keep prose for lessons about *judgment*, which cannot be asserted. *Up:* new invariant needed? PR it the same session, and pick the tier (unconditional → here; subsystem-triggered → `.claude/docs/invariants/` plus a routing line above). *Down:* anything here now conditional, duplicated, contradicted, or stale? Demote, merge, or fix it — **the downward pass is not optional.** Why all three, and the evidence → `.claude/docs/knowledge-layer.md`. `/gnite` runs the loop at session end; `/lesson` mid-session.
