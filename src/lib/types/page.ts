...
export interface OcrData extends ProcessingMetadata {
  ...
  /**
   * Set when `source === 'kraken'`: a specialist open OCR model read the page, because
   * the general model cannot read its script (Syriac, #4883 — scripts/lib/syriac-kraken-lane.mjs
   * is the table). `model_doi` is the Zenodo record of the model weights, so the reading is
   * reproducible and the provenance panel can cite it. Not Gemini output.
   */
  engine?: {
    name: string;                 // 'kraken'
    version: string;              // '7.1'
    model: string;                // 'sophro-mhiro' | 'omnisyr' | …
    model_label: string;
    model_doi: string;            // '10.5281/zenodo.17406773'
    licence: string;
