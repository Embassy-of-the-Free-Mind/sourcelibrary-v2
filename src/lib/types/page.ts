...
export type ContentSource =
  ...
  | 'manual'
  /**
   * The Internet Archive's own OCR of the same leaf (`<id>_djvu.xml`), taken because it
   * agreed with our model's reading of the book's sample pages. Not model output; see
   * `OcrData.ia` for who read it and when.
   */
  | 'ia_djvu'
  /**
   * Read by a specialist open OCR model (Kraken) on our own CPU, because the general
   * model cannot read the script — Syriac, #4883. Machine output, but not Gemini's:
   * exclude from Gemini quality measurement. `OcrData.engine` says which model.
   */
  | 'kraken'
  /**
   * Read by one of BDRC's specialist Tibetan OCR models (the Yigdzin VLM, or the
   * tibetan-ocr-app ONNX line models) on our own hardware, because the general model
   * fabricated Sanskrit scripture on these pages (#4523). Machine output, but not
   * Gemini's: exclude from Gemini quality measurement. `OcrData.engine` says which
   * model and revision; `OcrData.verdict` says why the page was served or marked.
   */
  | 'bdrc'
...
export interface OcrData extends ProcessingMetadata {
  ...
  data: string;
  /**
   * Set when `source === 'ia_djvu'`: the text is the Internet Archive's own OCR of the
   * same leaf, taken because it agreed with our model's reading of the book's sample
   * pages (scripts/import/ia-ocr-ingest.mjs). Not model output — exclude from any
   * Gemini quality measurement. The reader's provenance panel renders this block.
   */
  ia?: {
    item: string;                 // archive.org identifier
    engine: string | null;        // 'ABBYY FineReader 8.0' | 'Tesseract' | null when the item does not say
    module_version: string | null;
    converter: string | null;     // e.g. 'abbyy-to-hocr 1.1.11'
    ocr_date: Date | null;        // mtime of the item's _djvu.xml — when the Archive generated the text
    scandate: string | null;      // YYYYMMDD
  ...
  /**
   * Set when a specialist open OCR model read the page, because the general model cannot
   * read its script. Describes the model that produced `data` — nothing else.
   * - `source === 'kraken'` (Syriac, #4883 — scripts/lib/syriac-kraken-lane.mjs is the table):
   *   `model_doi` is the Zenodo record of the weights; the Kraken-only fields are set.
   * - `source === 'bdrc'` (Tibetan, #4523/#4722 — scripts/maintenance/apply-reocr-verdicts.mjs
...
/**
 * Why a translation was withheld. Values are produced by
