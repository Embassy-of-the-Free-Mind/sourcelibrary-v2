    ...
    <>
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            <div>
              ...
              <h1 className="text-2xl sm:text-3xl font-display font-bold leading-tight">
                {titleInfo.display}
                {titleInfo.isDescriptive && (
                  <AiBadge
                    className="ml-2 align-super"
                    title={
                      titleInfo.model
                        ? `Descriptive label generated by ${formatModelName(titleInfo.model)} from the image — not a published title`
                        : 'Descriptive label generated by AI from the image — not a published title'
                    }
                  />
                )}
              </h1>
              {/* The line under the title used to be an unlabelled grey italic
                  echo of `title`, which read as a subtitle. On a record whose
                  <h1> is an AI description of the picture (#4288) that line is
                  the only true bibliographic title we hold, so it has to say
                  which is which — otherwise the invented label is what gets
                  cited. */}
              {titleInfo.isDescriptive ? (
                <p className="text-stone-400 mt-1.5 text-sm">
                  <span className="uppercase tracking-wider text-[10px] text-stone-500 mr-1.5">Title on the source record</span>
              ...
              {titleInfo.isDescriptive && (
                <p className="text-stone-500 mt-1.5 text-xs max-w-prose">
                  The heading above is a description of the image generated by AI, not a title this
                  work was published under. Cite it by the source record&rsquo;s title.
                </p>
              )}
            ...
            <div className="flex items-center gap-1 rounded-lg bg-white/5 px-1 py-0.5 flex-shrink-0">
              ...
              />
              {/* The share menu's only use of `title` is the citation string it
                  copies/tweets, so it takes the source record's title — never an
                  AI description of the image (#4288). */}
              <BookShare
                title={titleInfo.citation}
                author={book.author}
      ...
      <div className="max-w-[var(--container-standard)] mx-auto px-6 md:px-12 py-10 space-y-8">
        ...
        <div className="card p-6 sm:p-8">
          ...
          {(enrichment?.ulan_artist || enrichment?.tgn_place || enrichment?.aat_technique || enrichment?.aat_style) && (
