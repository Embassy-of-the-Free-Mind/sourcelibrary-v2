...
const ACCENT: Record<LibrarySectionData['color'], string> = {
  rust: '#a5503d', gold: '#9a7526', sage: '#5f6f4e', violet: '#6a5b8a',
};
 ...
 * came from. Light editorial layout: the library's logo (or name), a short
 * description and stats on one side; a strip of that library's book covers on
 * the other (falling back to a single representative image).
 */
export default function BookLibrarySection({ data }: { data: LibrarySectionData }) {
  ...
  const stats = [
    { label: 'books here', value: fmt(data.stats.books) },
        ...
        <div className="grid md:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] gap-9 md:gap-14 items-center">
          ...
          <div>
            ...
            {data.logo ? (
              ...
              <img src={data.logo} alt={data.name} className="h-9 md:h-11 w-auto max-w-[280px] object-contain object-left mb-4" loading="lazy" />
            ) : (
