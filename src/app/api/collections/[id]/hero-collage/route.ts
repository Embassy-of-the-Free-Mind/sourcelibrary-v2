...
import { weaveBySubject, dedupeImages, nearDupeSignature, topicTermsFromName, type RankableImage } from '@/lib/collection-image-ranking';
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const pool = await db.collection('gallery_images').find(
      ...
      { projection: { _id: 0, thumbnail_url: 1, extracted_url: 1, image_url: 1, type: 1, description: 1, museum_description: 1, gallery_quality: 1, book_id: 1, book_title: 1 }, maxTimeMS: 5000 },
    ...
    const unique = dedupeImages(dedupeImages(pool as Array<Record<string, unknown>>, (g) => urlOf(g)), nearDupeSignature);
