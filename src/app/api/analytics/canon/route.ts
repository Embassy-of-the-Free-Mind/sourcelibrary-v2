...
export const GET = withAuth(async () => {
  try {
    ...
    const [
      ...
      // 3. "First translations" — non-English books with any translation
      // These are texts that were never available in English before
      db.collection('books').aggregate([
        {
          $match: {
            hidden: { $ne: true },
        ...
        },
        {
          $group: {
            ...
            fully_translated: {
              $sum: {
                $cond: [
                  { $gte: ['$pages_translated', { $multiply: [{ $ifNull: ['$pages_ocr', 0] }, 0.9] }] },
                  1, 0,
                ],
              },
            },
    ...
    const ft = firstTranslations[0] || { count: 0, fully_translated: 0, pages: 0 };
    ...
    const data = {
      ...
      canon: {
        ...
        first_translations_complete: ft.fully_translated,
