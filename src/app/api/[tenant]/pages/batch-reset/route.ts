...
export const POST = withAuth(async (request, session, context) => {
  try {
    ...
    const pages = await db.collection('pages')
      .find({ id: { $in: pageIds }, crop: { $exists: true }, tenantId })
      .toArray();
    ...
    const siblings = await db.collection('pages')
      .find({ split_from: { $in: pageIdsToReset }, tenantId })
      .toArray();
    ...
    await Promise.all([
      siblingIds.length > 0
        ? db.collection('pages').deleteMany({ id: { $in: siblingIds }, tenantId })
        : Promise.resolve(),
      db.collection('pages').updateMany(
        { id: { $in: pageIdsToReset }, tenantId },
        {
