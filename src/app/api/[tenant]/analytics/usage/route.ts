...
export const GET = withAuth(async (request: NextRequest, session, context) => {
  try {
    ...
    const [snapshot, recentBooksResult, geminiUsageFacetArr, batchJobsResult, highFailureBooks] = await Promise.all([
      ...
      db.collection('system_config').findOne({ _id: 'analytics_usage', tenantId } as any).catch(() => null),
      ...
      db.collection('batch_jobs').aggregate([
        { $match: { tenantId, status: { $in: ['pending', 'processing'] } } },
        { $group: { _id: { status: '$status', type: '$type' }, count: { $sum: 1 } } },
      ]).toArray().catch(() => []),

      // 5. High-failure-rate OCR jobs
      db.collection('jobs').aggregate([
        { $match: { tenantId, type: 'ocr', status: 'completed_with_errors', updated_at: { $gte: cutoffDate } } },
