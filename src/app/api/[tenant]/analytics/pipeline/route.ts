...
export const GET = withAuth(async (request: NextRequest, session, context) => {
  try {
    ...
    const [snapshots, cronRuns, needsAttention, recentErrors, recentDecisions, velocityRows] = await Promise.all([
      ...
      db.collection('books')
        .find({ tenantId, 'pipeline_auto.status': { $in: ['needs_attention', 'failed'] } })
        .project({
    ...
    const snapshot = await db.collection('system_config').findOne({ _id: 'enrichment_snapshot', tenantId } as any)
      || await db.collection('system_config').findOne({ _id: 'enrichment_snapshot' as any });
