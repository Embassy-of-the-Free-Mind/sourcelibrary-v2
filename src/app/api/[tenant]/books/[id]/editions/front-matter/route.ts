...
const genAI = () => (_genAI ??= getGeminiClient({ endpoint: '/api/[tenant]/books/[id]/editions/front-matter', type: 'other' }));
...
export const POST = withAuth(async (request, session, context) => {
  try {
    ...
    const indexDoc = await getBookIndexFields(db, bookId, FRONT_MATTER_INDEX_FIELDS, { tenantId });
    if (indexDoc) {
