...
async function executeSummarize(
  ...
  try {
    ...
    const indexResponse = await fetch(`${baseUrl}/api/${tenantSlug}/books/${bookId}/index`, {
      method: 'GET',
...
export const POST = withAuth(async (request, session, context) => {
  try {
    ...
    await updatePipeline(db, bookId, tenantId, {
    ...
    switch (step) {
      ...
      case 'ocr': {
        const ocrResult = await createOcrJob(bookId, tenantId, pipeline.config, db);
      ...
      case 'translate': {
        const translateResult = await createTranslateJob(bookId, tenantId, pipeline.config, db);
      ...
      case 'summarize':
        stepResult = await executeSummarize(bookId, tenantId, tenant, pipeline.config, db);
        break;

      case 'edition':
        stepResult = await executeEdition(bookId, tenantId, pipeline.config, db);
        break;
    ...
    await updatePipeline(db, bookId, tenantId, stepUpdates);
