 ...
 * Tenant My Likes API
 *
 * GET /api/[tenant]/likes/mine - Get all likes for a visitor with enriched data
 */
...
export async function GET(
  ...
  try {
    ...
    const myLikes = await db.collection('likes')
      .find({ visitor_id: visitorId, target_type: targetType, tenantId })
    ...
    const countPipeline = [
      { $match: { tenantId, target_type: targetType, target_id: { $in: targetIds } } },
    ...
    if (targetType === 'book') {
      ...
      const galleryImages = await db.collection('gallery_images').aggregate([
        { $match: { tenantId, book_id: { $in: targetIds }, gallery_quality: { $gte: 0.6 }, extracted_url: { $exists: true, $ne: null } } },
