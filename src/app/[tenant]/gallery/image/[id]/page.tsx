...
function getCacheKey(id: string, tenant: string): string {
  return `${tenant}:${id}`;
}

function prefetchApiResponse(id: string, tenant: string) {
  const key = getCacheKey(id, tenant);
  ...
  setTimeout(() => prefetchCache.delete(key), 120000);
}

async function getCachedOrFetch(id: string, tenant: string): Promise<GalleryImageDetail> {
  const key = getCacheKey(id, tenant);
...
export default function ImageDetailPage({
  ...
  useEffect(() => {
    ...
    async function fetchImage() {
      try {
        const json = await getCachedOrFetch(imageId!, tenant);
        setData(json);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (currentIndex < bookImageIds.length - 1) prefetchApiResponse(bookImageIds[currentIndex + 1], tenant);
      // Prefetch prev after a longer delay
      if (currentIndex > 0) prefetchApiResponse(bookImageIds[currentIndex - 1], tenant);
    }, 500);
    return () => clearTimeout(timer);
  }, [currentIndex, bookImageIds, tenant]);
  ...
  const navigateTo = useCallback(async (index: number) => {
    ...
    window.history.replaceState(null, '', `/${tenant}/gallery/image/${imgId}`);
    ...
    try {
      ...
      const json = await getCachedOrFetch(imgId, tenant);
        ...
        <div className="max-w-5xl mx-auto px-5 sm:px-8 py-8 sm:py-10 overflow-hidden">
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="space-y-5 min-w-0">
              ...
              {imageId && <SimilarImages imageId={imageId} tenant={tenant} />}
            </div>
          </div>
        </div>
      </div>
