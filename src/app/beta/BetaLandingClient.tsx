...
export default function BetaLandingClient({ stats }: { stats: BetaStats }) {
      ...
      <section className="py-16 md:py-20 bg-stone-900">
        <div className="px-6 md:px-12 max-w-[1500px] mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8 text-center">
            {/* Books / Languages / Illustrations are live from the homepage_stats
                cache (passed in via props). Pages scanned/translated are page-level
                aggregates not in that cache — kept as manually-maintained estimates. */}
            {[
              { number: stats.books, label: 'Rare books' },
              { number: '3M+', label: 'Pages scanned' },
              { number: '1M+', label: 'Pages translated' },
              { number: stats.languages, label: 'Languages' },
              { number: stats.illustrations, label: 'Illustrations' },
            ].map((stat) => (
              <div key={stat.label}>
                <div
