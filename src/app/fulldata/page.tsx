...
import { CenturyChart } from '../data/DataCharts';
...
const PIPELINE_STATUS_COLORS: Record<string, string> = {
  queued: 'bg-stone-200',
  archiving: 'bg-amber-200',
  archive_complete: 'bg-amber-300',
  ocr_submitted: 'bg-sky-200',
  ocr_complete: 'bg-sky-300',
  metadata_enriched: 'bg-sky-400',
  translate_submitted: 'bg-violet-200',
  translate_complete: 'bg-violet-300',
  enriching: 'bg-violet-400',
  enriched: 'bg-violet-500',
  chapters: 'bg-emerald-200',
  chapters_complete: 'bg-emerald-300',
  images_submitted: 'bg-amber-400',
  images_complete: 'bg-amber-500',
...
async function fetchFullData(): Promise<FullData> {
  ...
  const [
    ...
    // Languages (all books, all languages)
    books
    ...
    books
      .aggregate<{ _id: string; count: number }>([
        { $match: { 'image_source.provider_name': { $exists: true } } },
...
export default async function FullDataPage() {
  ...
  const maxLangCount = data.languages[0]?.count ?? 1;

  const stats = [
    { value: formatNumber(data.totalBooks), label: 'Total books', sub: `${formatNumber(data.hiddenCount)} hidden` },
    { value: formatNumber(data.totalPages), label: 'Total pages' },
    { value: formatNumber(data.totalOcr), label: 'Pages with OCR', sub: pct(data.totalOcr, data.totalPages) },
    ...
    <ContentPageLayout
      header={
        <ContentHeader
          ...
          subtitle={`${formatNumber(data.totalBooks)} books (${formatNumber(data.hiddenCount)} hidden) across ${uniqueLanguages} languages, from the ${earliestCentury} to the ${latestCentury}`}
        />
      }
      maxWidth="wide"
    >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
          {stats.map((s) => (
            ...
            >
              ...
              {s.sub && <div className="text-faint text-xs mt-0.5">{s.sub}</div>}
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ── OCR & Translation Coverage Tiers ── */}
      <section className="mb-16">
