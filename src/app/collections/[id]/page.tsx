...
async function fetchCollectionData(id: string, provider?: string) {
  ...
  function toPlain<T>(value: T): T {
    ...
    if (allBookIds.size > 0) {
      ...
      exhibitionBooks = exBooks.map(({_id, ...b }) => ({
        ...b,
      ...
      total,
      // Strip Mongo-specific values (_id/ObjectId) so client components receive plain objects only.
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      galleryImages: toPlain((galleryImages as Record<string, unknown>[]).map(({_id, ...rest }) => rest)) as any[],
      mentionedBooks: sanitizeBookThumbs(mentionedBooks) as unknown as BookItem[],
      parentCollection,
      galleryCollectionSlug,
      galleryTotalCount,
      exhibition: toPlain(curationDraft?.curation || null),
      exhibitionBooks: toPlain(exhibitionBooks),
