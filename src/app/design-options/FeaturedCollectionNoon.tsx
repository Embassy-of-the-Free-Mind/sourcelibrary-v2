...
export interface NoonFeaturedCollectionItem {
  collection: {
    slug: string;
    name: string;
    subtitle: string;
...
export default function FeaturedCollectionNoon({ items }: { items: NoonFeaturedCollectionItem[] }) {
  ...
  return (
    <section className="bg-dark py-12 md:py-16 relative overflow-hidden">
      <div className="px-6 md:px-12 max-w-[1500px] mx-auto overflow-hidden">
                ...
                return (
                  <Link key={book.id} href={bookUrl(book)} className="group">
                    <div className="aspect-[3/4] relative rounded-lg overflow-hidden bg-white/5 border border-white/10 group-hover:border-white/30 transition-all">
                      {thumb ? (
                        <Image src={thumb} alt={bookTitle(book)} fill quality={85} className="object-cover group-hover:scale-105 transition-transform duration-300" sizes="(max-width: 640px) 22vw, 12vw" />
                      ) : (
                        <div className="absolute inset-0 flex items-center justify-center"><BookOpen className="w-8 h-8 text-white/20" /></div>
                      )}
                    </div>
                    <p className="text-xs text-white/50 mt-1.5 line-clamp-2 group-hover:text-white/70 transition-colors leading-tight">{bookTitle(book)}</p>
                  </Link>
                );
              })}
            </div>
