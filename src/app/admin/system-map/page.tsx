...
const serviceData: Record<string, ServiceInfo> = {
  ...
  pipeline: {
    label: 'Pipeline Orchestrator', subtitle: 'Hetzner · every 10min', color: '#d63031', icon: '🔄',
    url: 'https://sourcelibrary.org/admin/pipeline',
    summary: 'The central pipeline that moves books from raw imports through OCR, translation, and image extraction. It is ONE unified pipeline with three parallel phases, NOT three separate pipelines. Runs on a Hetzner cax31 ARM server, not on Vercel.',
    details: [
      'The post-import-pipeline cron runs every 10 minutes on Hetzner (46.224.122.120). It scans for books that need processing, creates job records in the jobs collection, and enqueues page messages to the appropriate SQS queues.',
      'Phase 1 — OCR: Pages are sent to the pageOcr FIFO queue. Processing is parallel and batch-friendly. Each page is independently OCR\'d by Gemini, so pages can be processed in any order.',
      'Phase 2 — Translation: Pages are sent to the pageTranslation FIFO queue. Processing is SEQUENTIAL because each page\'s translation needs context from the previous page to maintain narrative continuity. This is why translation uses the realtime API, never the Batch API.',
      'Phase 3 — Image Extraction: Pages are sent to the pageImageExtraction queue. Gemini Vision analyzes each page for illustrations, diagrams, and decorative elements. Processing is parallel.',
      'All three phases write results to the writeResults SQS queue, where the write-processor Lambda batches MongoDB writes to avoid overwhelming Atlas.',
      'Backpressure: MongoDB Atlas saturates at ~40 concurrent Lambda connections. Per-phase limits: OCR 20, translation 30, images 50. The adaptive-limits system dynamically adjusts these based on DB response times.',
      'The orchestrator on Hetzner is NOT in git — it\'s a local crontab entry. This is a known gap in the infrastructure-as-code story.',
    ],
    gotchas: [
      'paused: true stops new job queueing but does NOT stop in-flight Lambda workers. To actually stop work, you must CANCEL jobs in MongoDB.',
      'Hetzner crons are not in git. If the server is rebuilt, cron config must be manually recreated.',
      'The pipeline code still exists as Vercel API routes (src/app/api/cron/*) but is invoked from Hetzner via HTTP, not from Vercel\'s cron scheduler.',
      'Stale Vercel connection pools after DB recovery — must redeploy Vercel to reset.',
    ],
    files: [
      'src/app/api/cron/post-import-pipeline/ — orchestrator logic',
      'src/app/api/jobs/queue-books/ — job queueing endpoint',
      'src/lib/adaptive-limits.ts — dynamic backpressure',
      'src/lib/sqs-client.ts — SQS message sending',
    ],
    collections: ['jobs', 'batch_jobs', 'system_config', 'gemini_usage'],
  },
  enrich: {
    label: 'Enrichment', subtitle: 'Hetzner · every 10min', color: '#d63031', icon: '✨',
    url: 'https://sourcelibrary.org/admin/pipeline',
    summary: 'Auxiliary pipeline phase that runs alongside the main OCR/translation pipeline. Enriches books with AI-generated metadata: summaries, chapter extraction, searchable indexes, and transliteration of non-Latin scripts.',
    details: [
      'Runs on the same Hetzner server as the main pipeline, also every 10 minutes.',
      'Summary generation: Uses Gemini to create a 2-3 paragraph scholarly summary of the book based on OCR text. Stored in the book document.',
      'Chapter extraction: Identifies chapter/section breaks in the OCR text and creates a navigable table of contents.',
      'Index generation: Creates a searchable keyword index for each book — people, places, concepts, page references.',
      'Transliteration: For books in non-Latin scripts (Hebrew, Arabic, Cuneiform), generates Latin-alphabet transliterations alongside the original.',
      'All enrichment uses gemini-3-flash-preview. This was a recurring issue — older models were accidentally used in the past.',
    ],
    gotchas: [
      'ALWAYS use gemini-3-flash-preview for enrichment. Do not use older models.',
      'Enrichment only runs on books that have completed OCR. It skips books still in the OCR phase.',
    ],
    files: [
      'src/app/api/cron/enrich-books/ — enrichment cron logic',
      'src/lib/metadata-enrichment.ts — AI enrichment (575 lines)',
      'src/lib/ai.ts — core Gemini operations (506 lines)',
    ],
  },
  sync: {
  ...
  sqs_ocr: {
    label: 'pageOcr Queue', subtitle: 'FIFO · parallel', color: '#ff9f43', icon: '📬',
    url: 'https://eu-central-1.console.aws.amazon.com/sqs/v3/home?region=eu-central-1',
    summary: 'SQS FIFO queue for OCR jobs. Pages can be processed in any order since OCR is stateless — each page is independently converted from image to text.',
    details: [
      'Message format: PageOcrMessage with pageId, bookId, imageUrl, and processing options.',
      'FIFO queue ensures no duplicate processing (deduplication by pageId).',
      'Batch sending: Up to 10 messages per SQS batch call.',
      'Consumer: sourcelibrary-ocr-processor Lambda (reserved concurrency: 10).',
    ],
    files: ['src/lib/sqs-client.ts'],
  },
  sqs_trans: {
    label: 'pageTranslation Queue', subtitle: 'FIFO · sequential', color: '#ff9f43', icon: '📬',
    url: 'https://eu-central-1.console.aws.amazon.com/sqs/v3/home?region=eu-central-1',
    summary: 'SQS FIFO queue for translation jobs. Unlike OCR, translation is SEQUENTIAL — each page needs the previous page\'s translation as context to maintain narrative flow. The FIFO ordering guarantees pages are processed in page_number order.',
    details: [
      'Message group ID is the bookId, ensuring all pages of a book are processed in order.',
      'Each translation message includes the previous page\'s translated text for context continuity.',
      'This is why translation MUST use the realtime Gemini API, never the Batch API — batch processing can\'t guarantee page ordering.',
  ...
  lambda_ocr: {
    ...
    summary: 'Lambda worker that converts page scan images to text using Gemini Vision. Processes one page per invocation: downloads the image, sends it to Gemini with the Standard OCR prompt (v6), and writes the result to the writeResults queue.',
    details: [
      'Input: SQS message with pageId and imageUrl. Downloads the page image from R2/archive source.',
      'Processing: Sends image to Gemini with Standard OCR prompt v6. The prompt handles all languages — there are NO language-specific prompts (this was a resolved issue).',
      'Output: Sends WriteResultMessage to the writeResults SQS queue with the OCR text, content hash, and any detected_images metadata.',
      'Error handling: Retries with exponential backoff on Gemini rate limits (429). Dead-letters after 3 failures.',
      'Reserved concurrency: 10 Lambda instances. Backpressure limit (from adaptive-limits): 20 concurrent.',
      'Deployment: TypeScript source compiled with esbuild, packaged as zip, deployed via AWS CLI.',
    ],
    gotchas: [
  ...
  lambda_trans: {
    ...
    summary: 'Lambda worker that translates OCR text into English using Gemini. Unlike OCR, translation is sequential — each page receives the previous page\'s translation as context to maintain narrative continuity across the book.',
    details: [
      'Input: SQS message with pageId, bookId, OCR text, and the previous page\'s translation for context.',
      'Processing: Sends OCR text + previous page context to Gemini for translation. The prompt instructs Gemini to maintain consistent terminology, names, and narrative flow across pages.',
      'Context chain: This is why translation uses a FIFO queue with bookId as the message group ID — pages MUST be translated in order. Breaking order produces incoherent translations.',
      'Output: Writes translation to the writeResults queue. Also updates the page\'s translation.data directly (hybrid write pattern).',
      'Reserved concurrency: 15 Lambda instances. Backpressure limit: 30 concurrent.',
      'Realtime API ONLY: The Gemini Batch API processes requests in arbitrary order, which breaks the context chain. This caused a major quality incident and is now a hard rule.',
    ],
    gotchas: [
      'CRITICAL: NEVER use Gemini Batch API for translation. It processes pages out of order, breaking context continuity.',
      'If a single page fails mid-book, subsequent pages will have a context gap. The retry logic re-translates from the last successful page.',
    ],
    files: [
  ...
  gemini: {
    ...
    summary: 'Google\'s Gemini is the AI backbone for everything: OCR, translation, image extraction, enrichment (summaries, indexes, chapters), and classification. Uses gemini-3-flash-preview for ALL tasks. 10 API keys rotate with 60s cooldown to handle rate limits.',
    details: [
      'Model: gemini-3-flash-preview is used for ALL tasks. This was standardized after incidents with older models producing lower-quality output. Do not use other models without explicit decision.',
      'Key rotation: 10 API keys (GEMINI_API_KEY through GEMINI_API_KEY_10). When one key hits a rate limit (429), the client rotates to the next key and puts the exhausted key on 60s cooldown. The gemini-client.ts manages this automatically.',
      'GEMINI_API_KEY_TIER3: A separate high-quota key reserved for heavy batch work. Used by scripts that process hundreds of pages.',
      'Batch API: Supported for OCR (50% cheaper) but NEVER for translation (see translation processor gotchas). Batch jobs are only visible to the API key that created them — multi-key batch support requires careful key tracking.',
      'Cost tracking: Every API call is logged to the gemini_usage collection with model, input/output tokens, estimated cost, and latency. gemini_usage_daily has daily aggregates. This is the single source of truth for AI costs.',
      'Prompt management: OCR uses Standard OCR v6 prompt. Translation prompt includes context from the previous page. Enrichment prompts are in metadata-enrichment.ts. Some prompts are also stored in the prompts collection for A/B testing.',
    ],
    gotchas: [
      'ALWAYS use gemini-3-flash-preview. This was a recurring issue.',
  ...
  crons: {
    label: 'Vercel Crons', subtitle: '4 active · 7 disabled', color: '#f0f6fc', icon: '⏰',
    summary: 'Vercel\'s cron scheduler triggers 4 active background jobs. Seven former pipeline crons have been moved to Hetzner but their API route code still exists in the codebase (disabled in vercel.json).',
    details: [
      'social-post (every 3h): Picks the next queued tweet from social_posts and publishes via Twitter API. Respects daily post limit.',
      'health-check (hourly): Monitors MongoDB latency, checks for zombie jobs (stuck in "processing" > 2h), alerts on connection pool pressure. Writes to pipeline_health_daily.',
      'daily-pipeline-report (6 AM UTC): Comprehensive daily snapshot — books processed, pages OCR\'d, translation progress, error rates, cost summary. Writes to pipeline_health_daily.',
      'social-reset (daily midnight UTC): Resets the daily tweet counter. On the 1st of each month, also resets monthly metrics.',
      'Disabled routes (code exists, not in vercel.json): submit-batch-ocr, process-batches, sync-page-counts, sync-gallery-images, enrich-books, post-import-pipeline, archive-ocr. All moved to Hetzner.',
    ],
    gotchas: [
      '7 disabled cron routes still exist in the codebase. They\'re invoked from Hetzner via HTTP — the code is shared, only the scheduler moved.',
      'Vercel crons have a max timeout of 60s (Hobby) or 300s (Pro). Heavy processing was moved to Hetzner because of this limit.',
    ],
    files: [
      'src/app/api/cron/ — all 11 cron route handlers',
      'vercel.json — cron schedule configuration (4 active entries)',
    ],
  ...
  scripts: {
    ...
    details: [
      ...
      'Batch processing (~33): Direct Gemini API calls for bulk OCR, translation, re-OCR. Bypass the Lambda pipeline for speed. Examples: realtime-ocr.mjs (Tier 3 keys, high RPS), submit-translations.mjs, retranslate-stale.mjs.',
    ...
    files: [
      ...
      'scripts/workers/ — sync-worker.mjs (Hetzner)',
    ],
  },
};
