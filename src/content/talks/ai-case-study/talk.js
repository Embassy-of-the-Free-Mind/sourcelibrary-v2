...
if($('#cost-nums'))$('#cost-nums').innerHTML=`<div class="stat"><div class="v">Cents</div><div class="l">a page to read and translate; a typical book costs less than a coffee</div></div><div class="stat"><div class="v">8 June</div><div class="l">the day we paused the whole pipeline to control spend</div></div><div class="stat"><div class="v">8 August</div><div class="l">back on, behind a daily budget dial that is closed by default</div></div>`;
...
columns('#ch-cost-month',D.costMonthly.map(m=>[m[0],m[1],m[0]>='2026-07'?'ctx':'yellow']),{title:'Model spend by month, relative to the peak month',lab:s=>({'01':'Jan','02':'Feb','03':'Mar','04':'Apr','05':'May','06':'Jun','07':'Jul','08':'Aug','09':'Sep'})[s.slice(5)],fmt:v=>v+'%',marks:[{i:5,t:'paused'},{i:7,t:'budget dial'}],legend:'<span><i class="yellow"></i>pipeline running</span><span><i class="yellow-tint"></i>paused or budget-capped</span>'});
...
(function(){
 ...
 function renderSteps(list){stepsEl.innerHTML=list.map((s,i)=>`<button class="seg" type="button"><span class="n">${String(i+1).padStart(2,'0')}</span><span class="t">${s.name}</span><i></i></button>`).join('');[...stepsEl.children].forEach((b,i)=>b.addEventListener('click',()=>{go(i);setPlaying(true)}))}
 let go=function(i){cur=i;if(version==='circles'){const st=steps[i];gs.forEach(g=>{const d=all.find(x=>x.name===g.dataset.name);g.classList.toggle('on',st.show(d))});cap.innerHTML=st.cap;const tb=bbox(all.filter(st.show),st.halos);relabel(tb,st.halos);setVB(tb,vb?1800:0)}
  else{renderScale(i)}
  [...stepsEl.children].forEach((s,k)=>{s.classList.toggle('is',k===i);s.classList.toggle('done',k<i)})};
 ...
 // autoplay: each step holds for DUR ms while its button fills; arrow keys or clicks restart the clock
 const DUR=5500;let playing=false,t=0,last=0,onScreen=false;const playBtn=host.querySelector('.bub-play');
 function setPlaying(on){playing=on;host.querySelector('.bubbles').classList.toggle('paused',!on);if(on){last=performance.now()}}
 playBtn.addEventListener('click',()=>setPlaying(!playing));
 const _go=go;go=function(i){_go(i);t=0;[...stepsEl.children].forEach((b,k)=>b.lastElementChild.style.setProperty('--w',k<i?'100%':'0%'))};
 (function frame(now){requestAnimationFrame(frame);const dt=Math.min(120,now-last);last=now;if(!playing||!onScreen)return;t+=dt;const list=version==='circles'?steps:barSteps;const seg=stepsEl.children[cur];if(seg)seg.lastElementChild.style.setProperty('--w',Math.min(100,t/DUR*100)+'%');if(t>=DUR){if(cur<list.length-1)go(cur+1);else setPlaying(false)}})(performance.now());
 new IntersectionObserver(es=>{es.forEach(e=>{onScreen=e.isIntersecting;if(onScreen){setPlaying(!RM)}else setPlaying(false)})},{threshold:.6}).observe(host);
 ...
 window.gapNext=()=>{const list=version==='circles'?steps:barSteps;if(cur<list.length-1){go(cur+1);setPlaying(true)}else if(version==='bars')setVersion('circles')};window.gapSetVersion=setVersion;
