...
# Usage: secret-lover run -- bash scripts/run-bulk-reocr.sh [--dry-run] [--offset=N] [--limit=N]
#
...
echo "=== Bulk Re-OCR: Starting (offset=$OFFSET, limit=$LIMIT) ==="
echo ""

while true; do
  echo "--- Calling offset=$OFFSET ---"
  RESPONSE=$(curl -s --max-time 310 -H "Authorization: Bearer $CRON_SECRET" \
    "${BASE_URL}?offset=${OFFSET}&limit=${LIMIT}${DRY_RUN}" 2>&1)
  ...
  # No pause — tier 3 key has no rate limits
done
