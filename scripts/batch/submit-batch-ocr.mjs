...
async function main() {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    let result;
    try {
      result = await submitBatchOcr(id, cronSecret, keyIndex);
    } catch (fetchErr) {
      console.log(`NETWORK ERROR: ${fetchErr.message?.slice(0, 80)}`);
      failed++;
      // Brief pause then continue — transient network errors shouldn't kill the whole run
