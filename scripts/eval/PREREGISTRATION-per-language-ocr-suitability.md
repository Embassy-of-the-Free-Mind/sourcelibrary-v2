...
6. **Metric defect found at first scoring, fixed per the non-Latin invariant
   (post-run, disclosed).** The first scoring pass used `agreementPrimary()`
   from `lib/metrics.mjs`, whose word tokenizer replaces combining marks with
   separators. On Devanagari, Syriac and Ethiopic that shreds every word into
   sub-floor fragments and reads as ~0 agreement — the exact failure shape in
   `.claude/docs/invariants/non-latin-text-operations.md` ("an operation
   reduces non-Latin input to nothing, and the nothing is read as a negative").
   The primary metric is therefore **character agreement on letters+marks**
   (1 − Levenshtein/max over the first 3,000 characters — the CER analogue),
   and the threshold was re-derived by the SAME pre-declared procedure (p25 of
   lite↔flash agreement among pinned pages within 1 pp CER): **0.956**
   (beyond-1 pp pinned pages: median 0.52, p75 0.81, max 0.97, so one in ten
   clears it). Both metrics are stored per page (`agreement_with_flash`,
   `agreement_word`) so the reader can see what the change moved. Rows whose
   API call failed (image fetch, expired Vision token) are *not run*, never
   *empty* — an error is not an absence.
