CREATE TABLE IF NOT EXISTS artwork_embeddings (
  ...
  embedding vector(768), resource_type TEXT, thumbnail_url TEXT,
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()
);
CREATE INDEX IF NOT EXISTS artwork_embeddings_hnsw_idx ON artwork_embeddings USING hnsw (embedding vector_cosine_ops) WITH (m = 16, ef_construction = 64);
...
CREATE OR REPLACE FUNCTION match_artworks_semantic(
  query_embedding vector(768), match_threshold float DEFAULT 0.5, match_count int DEFAULT 20,
