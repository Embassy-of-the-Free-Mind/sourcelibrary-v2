#!/usr/bin/env node
/**
 * PRIOR ART: scripts/import/_tmp-canonical-gaps-direct.mjs — same IA metadata → makeBookDoc →
 * insertBookIfNew → pages shape, reused nearly verbatim (the manifest-driven, gate-first IA
 * direct importer). It does not fit as-is for two reasons: it hard-codes a laptop path for its
 * manifest (this one resolves the manifest next to the script so it runs unchanged on Hetzner,
 * where bulk imports belong — invariants/import-cost-and-egress.md), and it leaves every book to
 * Phase 0 auto-enrolment, which would put these Syriac volumes straight onto the OCR lane that
 * measured 40% looped garbage on Bar Hebraeus on 2026-09-16. This one HOLDS each book at insert
 * (scripts/lib/pipeline-hold.mjs, #4790's mechanism) until the Kraken Syriac trial settles the
 * OCR route. scripts/import/ia-bundle-import.mjs is the bundled-item pattern — not this shape.
 *
 * Syriac canon acquisitions (Derek, 2026-09-16: "yes, pull what you can").
 * Manifest: scripts/import/syriac-canon-manifest.json — human-curated from archive.org on
 * 2026-09-16; every identifier verified for: library scan with image derivatives, not
 * access-restricted, IIIF manifest present, imprint pre-1929.
 *
 * Lands HIDDEN (visible:false, hidden:true) and HELD (pipeline_auto.status 'held' with a marker
 * naming the release condition). Making them public is Derek's call after QA; releasing the hold
 * is the Syriac-OCR session's call — and because these books never had a pipeline status, the
 * release MUST pass `--to queued` (releaseBook's default restore is 'ocr_complete', which would
 * skip OCR entirely):
 *   node scripts/maintenance/hold-pipeline-books.mjs --release-held --reason syriac-ocr-lane-trial --to queued --apply
 *
 * text_role is deliberately NOT set (several volumes are translations; the classify-text-role
 * sweep owns that field). `language` is the EDITION's language (invariants/language-fields.md);
 * facing-text editions carry `languages[]` + `language_multi`.
 *
 *   set -a; source .env.production.local; set +a
 *   node scripts/import/syriac-canon-direct.mjs --dry-run
 *   node scripts/import/syriac-canon-direct.mjs --commit [--limit N] [--only <ia-id>]
 */
...
const LIMIT = li > 0 ? +process.argv[li + 1] : Infinity;
const oi = process.argv.indexOf('--only');
const ONLY = oi > 0 ? process.argv[oi + 1] : null;
const HERE = path.dirname(fileURLToPath(import.meta.url));
const MANIFEST = path.join(HERE, 'syriac-canon-manifest.json');
const OUT = path.join(HERE, '..', '..', 'scratchpad', 'syriac-acq', 'import-results.jsonl');
const IMPORTER = 'syriac-canon-2026-09-16';
const HOLD = {
  reason: 'syriac-ocr-lane-trial',
  release: 'The Kraken Syriac trial (Qoruyo / Sophro Mhiro, 2026-09-16) has chosen a Syriac OCR route; release with --to queued so OCR runs from scratch on that route.',
  source: IMPORTER,
};
...
const { BOOKS } = JSON.parse(fs.readFileSync(MANIFEST, 'utf8'));
...
async function main() {
  ...
  console.log(`manifest ${BOOKS.length} · already held by ia id: ${held.size} · mode ${COMMIT ? 'COMMIT' : 'DRY-RUN'}\n`);
  ...
  for (const b of BOOKS) {
    ...
    const h = await holdBook(db, realId, HOLD);
    if (h.outcome === 'held') heldOk++; else console.log(`WARN  ${b.id} — hold outcome ${h.outcome}`);
    const CHUNK = 500;
