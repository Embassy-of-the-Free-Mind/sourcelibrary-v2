...
const rows = readFileSync(`${HERE}/classified-jhana.jsonl`, 'utf8')
  .trim().split('\n').filter(Boolean).map((l) => JSON.parse(l));
console.log(`classified rows: ${rows.length}`);

let pts = rows.filter((r) => r.experiential && r.quote_verified && r.quote && r.stage !== 'none' && (r.fit === 'native' || r.fit === 'analogical'));
...
writeFileSync(`${HERE}/points-jhana.jsonl`, final.map((p) => JSON.stringify(p)).join('\n') + '\n');
