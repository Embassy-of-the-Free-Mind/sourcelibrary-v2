...
/* ---------- probe vectors ---------- */
const { dimensions } = JSON.parse(readFileSync(`${HERE}/probes-v2.json`, 'utf8'));
...
const CACHE = `${HERE}/probe-vectors-v2.json`;
...
for (const l of langs.slice(0, 6)) for (const e of ERAS) {
...
/* ---------- run ---------- */
const out = createWriteStream(`${HERE}/hits-v4.jsonl`);
const seen = new Set();
...
writeFileSync(`${HERE}/v4-summary.json`, JSON.stringify({
