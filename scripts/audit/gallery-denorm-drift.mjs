#!/usr/bin/env node
/**
 * PRIOR ART: scripts/audit/gallery-visibility-leak.mjs — the opposite direction
 * of the same field. That detector finds images from a HIDDEN book still
 * showing (book_visible stuck true, or a frozen gallery_collections snapshot).
 * This one finds images from a LIVE book wrongly hidden (book_visible ABSENT
 * or stuck false). Searched scripts/audit on 2026-09-22 for gallery/denorm/
 * book_visible audits; the leak detector is the only related file and checks
 * the other direction.
 *
