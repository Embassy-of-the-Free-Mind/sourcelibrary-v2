#!/usr/bin/env node
/**
 * PRIOR ART: scripts/audit/duplicate-fingerprint-groups.mjs — groups books
 * sharing a source fingerprint and reports the groups, but never scores
 * whether the copies in a group actually DIFFER in content (ocr/images/
 * translation) or ranks which live copy a reader is worse off landing on;
 * that is this script's Report 2. Searched scripts/audit on 2026-09-22 for
 * record-completeness/read-path/provider-union/licence-resolution audits;
 * nothing else on main checks whether a book's provider, licence, thumbnail
 * and categories are actually resolvable on the READ path.
 *
