...
export const INDEXES = [
  ...
  { collection: 'books', key: { 'pages_translated_es': 1 }, options: { 'name': 'books_pages_translated_es_partial_idx', 'partialFilterExpression': { 'pages_translated_es': { '$gt': 0 } } }, why: 'Counter branch of the Spanish-edition selection ({ $or: [{ pages_translated_es: { $gt: 0 } }, { language: … }] }) on /es/collections and the /es homepage counts (#5073, #5074). With this + books_language_idx the $or is an index union (277 docs examined, 16 ms) — but ONLY when the language branch is an indexable $in, which is what `localizedEditionFilterIndexed` (src/lib/localized.ts) resolves the /i regex into; with the raw regex the planner rejects the union and FETCHes all ~57K visible books (1,246 render timeouts in one week, four failed builds). Partial: ~110 books carry the counter.' },
  // ── books_warehouse ───────────────────────────────────────────
  ...
  // #4523 (found live 2026-09-18, created by the re-OCR lane's migration): the withhold sweep's two indexed arms and its resumability marker.
  { collection: 'pages', key: { 'ocr.pipeline': 1, 'ocr.updated_at': 1 }, options: { 'name': 'pages_ocr_pipeline_partial', 'partialFilterExpression': { 'ocr.pipeline': { '$exists': true } } }, why: 'Re-OCR lane provenance (#4523): STALE_CANDIDATE_FILTER arm 1 in scripts/lib/stale-translation.mjs selects pages a withholding lane rewrote; partial so the 20M-page collection is not indexed for a field 69K pages carry.' },
  { collection: 'pages', key: { 'ocr.unreadable': 1, 'book_id': 1 }, options: { 'name': 'pages_ocr_unreadable_partial', 'partialFilterExpression': { 'ocr.unreadable': true } }, why: 'STALE_CANDIDATE_FILTER arm 2 (#4523): pages whose transcription is flagged untrustworthy, so the withhold sweep and the reader agree on the set.' },
  { collection: 'pages', key: { 'translation_withheld.reason': 1, 'book_id': 1 }, options: { 'name': 'pages_translation_withheld_partial', 'partialFilterExpression': { 'translation_withheld.reason': { '$exists': true } } }, why: 'Withheld translations (#4523): the drift audit and the Supabase move read the withheld set from Mongo every run rather than from a frozen list.' },
  // #4927: the materialised stale-translation verdict. Written by the daily sweep
  // (scripts/maintenance/mark-stale-translations.mjs) and by a re-OCR lane that stamps it itself; cleared by every translation writer. Compound on book_id
  // so "which books have stale translations" (the consumer's first query) is served by the index.
  { collection: 'pages', key: { 'translation_stale.reason': 1, 'book_id': 1 }, options: { 'name': 'pages_translation_stale_partial', 'partialFilterExpression': { 'translation_stale.reason': { '$exists': true } } }, why: 'Stale-translation marker (#4927): MongoDB cannot compare two fields of one document through an index, so staleness (ocr.updated_at newer than translation.updated_at beyond STALE_MARGIN_MS) is materialised here and the re-translate consumer drains it by book. Partial: the set is ~16K of 5.4M translated pages.' },
