...
export const INDEXES = [
  ...
  { collection: 'analytics_events', key: { 'created_at': 1 }, options: { 'name': 'ttl_90d_created_at', 'expireAfterSeconds': 7776000 }, why: 'GDPR data retention — 90-day TTL on created_at. scripts/maintenance/create-ttl-indexes.mjs (originally named ttl_created_at_90d; live name differs slightly, same purpose).' },
  // ── analytics_pageviews ───────────────────────────────────────
  ...
  { collection: 'analytics_pageviews', key: { 'timestamp': 1 }, options: { 'name': 'ttl_90d_timestamp', 'expireAfterSeconds': 7776000 }, why: 'GDPR data retention — 90-day TTL. scripts/maintenance/create-ttl-indexes.mjs (ttl_timestamp_90d in the script; live name differs slightly, same purpose).' },
  // ── annotations ───────────────────────────────────────────────
  ...
  // ── api_usage ─────────────────────────────────────────────────
  { collection: 'api_usage', key: { 'ts': 1 }, options: { 'name': 'ts_1', 'expireAfterSeconds': 7776000 }, why: 'Public-API gate usage log — 90-day TTL on raw events. src/lib/api-usage.ts ensureIndexes(), created lazily on first write.' },
  ...
  { collection: 'cron_runs', key: { 'timestamp': 1 }, options: { 'name': 'cron_runs_ttl_30d', 'background': true, 'expireAfterSeconds': 2592000 }, why: 'Retention TTL, 30 days. scripts/maintenance/create-ttl-indexes.mjs family (no exact source script found for the 30d cron_runs variant specifically; consistent with the TTL pattern used elsewhere).' },
  // ── curation_drafts ───────────────────────────────────────────
  ...
  { collection: 'gemini_usage', key: { 'timestamp': 1 }, options: { 'name': 'gemini_usage_ttl_90d', 'background': true, 'expireAfterSeconds': 7776000 }, why: 'Retention TTL, 90 days. scripts/maintenance/create-ttl-indexes.mjs family — CRITICAL per the archived route: \'Without this, 1.4M doc collection scan causes 504 timeouts\' (that comment covers the companion gemini_usage_ts_idx, this is the TTL sibling).' },
  // ── gemini_usage_daily ────────────────────────────────────────
  ...
  { collection: 'loading_metrics', key: { 'timestamp': 1 }, options: { 'name': 'ttl_30d_timestamp', 'expireAfterSeconds': 2592000 }, why: 'Retention TTL variant on a Date `timestamp` field. scripts/maintenance/create-ttl-indexes.mjs family.' },
  { collection: 'loading_metrics', key: { 'received_at': 1 }, options: { 'name': 'ttl_received_at_30d', 'expireAfterSeconds': 2592000 }, why: 'Retention TTL on `received_at`. scripts/maintenance/create-ttl-indexes.mjs notes received_at may be numeric rather than Date — verify the field type before trusting this TTL to actually expire docs (db.loading_metrics.findOne({}, {received_at:1})).' },
  // ── mcp_tool_calls ────────────────────────────────────────────
  ...
  { collection: 'mcp_tool_calls', key: { 'ts': 1 }, options: { 'name': 'ts_1', 'expireAfterSeconds': 7776000 }, why: '90-day TTL. src/lib/mcp-usage.ts.' },
  // ── page_revisions ────────────────────────────────────────────
  ...
  // ── search_queries ────────────────────────────────────────────
  { collection: 'search_queries', key: { 'ts': 1 }, options: { 'name': 'ts_1', 'expireAfterSeconds': 7776000 }, why: '90-day TTL on the direct-search query log. src/lib/search-log.ts ensureIndexes().' },
  ...
  // ── uptime_checks ─────────────────────────────────────────────
  { collection: 'uptime_checks', key: { 'checked_at': 1 }, options: { 'name': 'checked_at_1', 'background': true, 'expireAfterSeconds': 2592000 }, why: '30-day retention TTL on uptime-monitor results. scripts/uptime-monitor.mjs.' },
