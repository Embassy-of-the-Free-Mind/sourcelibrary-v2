...
export const INDEXES = [
  ...
  { collection: 'analytics_pageviews', key: { 'timestamp': 1 }, options: { 'name': 'ttl_90d_timestamp', 'expireAfterSeconds': 7776000 }, why: 'GDPR data retention — 90-day TTL, still live in prod. Creator (scripts/maintenance/create-ttl-indexes.mjs) RETIRED per #2976 — outside the audited #2976 decision scope, live TTL stays until Derek decides.' },
  // ── annotations ───────────────────────────────────────────────
  ...
  { collection: 'loading_metrics', key: { 'timestamp': 1 }, options: { 'name': 'ttl_30d_timestamp', 'expireAfterSeconds': 2592000 }, why: 'Retention TTL variant on a Date `timestamp` field, still live in prod. Creator (scripts/maintenance/create-ttl-indexes.mjs) RETIRED per #2976 — outside the audited #2976 decision scope, live TTL stays until Derek decides.' },
  { collection: 'loading_metrics', key: { 'received_at': 1 }, options: { 'name': 'ttl_received_at_30d', 'expireAfterSeconds': 2592000 }, why: 'Retention TTL on `received_at`, still live in prod. Creator (scripts/maintenance/create-ttl-indexes.mjs) RETIRED per #2976. The retired script noted received_at may be numeric rather than Date — if so this TTL never expired anything anyway.' },
  // ── mcp_tool_calls ────────────────────────────────────────────
  ...
  { collection: 'mcp_tool_calls', key: { 'ts': 1 }, options: { 'name': 'ts_1', 'expireAfterSeconds': 7776000 }, why: '90-day TTL, still live in prod. NOTE: the lazy creator (src/lib/mcp-usage.ts) no longer creates TTLs per #2976 (no automated retention) — this collection was outside the audited #2976 decision scope, so the live TTL stays until Derek decides; swap would follow the swap-ttl-to-plain-indexes.mjs pattern.' },
  // ── page_revisions ────────────────────────────────────────────
  ...
  // ── search_queries ────────────────────────────────────────────
  { collection: 'search_queries', key: { 'ts': 1 }, options: { 'name': 'ts_1', 'expireAfterSeconds': 7776000 }, why: '90-day TTL on the direct-search query log, still live in prod. NOTE: the lazy creator (src/lib/search-log.ts) no longer creates TTLs per #2976 — outside the audited #2976 decision scope, live TTL stays until Derek decides.' },
  ...
  // ── uptime_checks ─────────────────────────────────────────────
  { collection: 'uptime_checks', key: { 'checked_at': 1 }, options: { 'name': 'checked_at_1', 'background': true, 'expireAfterSeconds': 2592000 }, why: '30-day retention TTL on uptime-monitor results, still live in prod. NOTE: the creator (scripts/uptime-monitor.mjs) no longer creates TTLs per #2976 — outside the audited #2976 decision scope, live TTL stays until Derek decides.' },
